...
const products: Product[] = [
  {
    ...
    mrp: 1999,
    price: 999,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSUbsLNViwkjs94thSLNIqPwErrzj_Blb7jyw&s",
    rating: 5,
...
interface ProductCardProps {
  ...
  // Optionally, you can pass a toggleWishlist function if needed.
}

function ProductCard({ product, isWishlisted = true }: ProductCardProps) {
  return (
    <div className="group relative overflow-hidden hover:shadow-lg bg-white">
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="p-2">
          ...
          <div className="flex items-baseline space-x-2 mt-2">
            ...
            <p className="text-green-600 text-sm font-semibold">50% off</p>
          </div>
        </div>
      </Link>

      {/* Wishlist Button - shown as filled heart since item is wishlisted */}
      <button
...
export function Wishlist() {
  // For demonstration, we use the full products array.
  // In a real app, you might filter for only wishlisted products.
  return (
